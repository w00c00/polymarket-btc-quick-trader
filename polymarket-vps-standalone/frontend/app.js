...
$("credentialGuideBtn").onclick = () => showGuide("凭证填写指南", `
  ...
  <p>这一页保存的是每个交易用户自己的 Polymarket 交易凭证。常见有三类：浏览器钱包地址、代理/Funder 地址、CLOB API 三件套。VPS 程序最稳定的方式是填写 Polygon 私钥 + Funder 地址 + 签名类型，然后由程序自动派生 CLOB API Key、Secret、Passphrase。</p>
  <ul>
    <li><b>Polygon 私钥在哪里拿</b>：从你连接 Polymarket 的钱包里导出，例如 MetaMask / Rabby 的账户详情里选择“导出私钥”。不要导出主钱包，建议新建一个专用交易钱包，只放策略本金。私钥通常是 0x 开头的一长串字符。</li>
    <li><b>Funder 地址是什么</b>：Funder 是 Polymarket 代理钱包/资金地址，不一定等于浏览器钱包地址。你把 USDC 充进 Polymarket 后，网页里持仓、余额实际归属的通常就是这个代理地址。签名类型不是 0 时，后端下单和持仓查询都依赖它。</li>
    <li><b>Funder 怎么获取</b>：优先在 Polymarket 网页的钱包/资金页面复制代理钱包地址；也可以打开 Polygonscan 查你浏览器钱包和 Polymarket 代理合约交互后的代理地址。最实用的校验方式是：填入 Funder 后解锁凭证，点“刷新余额/持仓”，如果能读到你网页里的持仓/余额，说明地址匹配。</li>
    <li><b>签名类型怎么选</b>：普通外部钱包直接签名通常是 0；Polymarket 代理钱包常见是 1、2 或 3。你现在这种“USDC 充进 Polymarket 网页账户”的用法，通常按代理钱包方式处理，需要 Funder，默认先用 3；如果下单报签名/授权错误，再按实际钱包类型调整。</li>
    <li><b>CLOB 三件套</b>：API Key、Secret、Passphrase 可以留空。留空时后端会用私钥调用 Polymarket CLOB 自动派生，长期运行更省心，也减少复制错误导致的 401。若你已经在别处生成过三件套，也可以三项一起填写，不能只填其中一两个。</li>
    <li><b>稳定性建议</b>：优先使用“私钥自主派生三件套”的方式。它减少手填错误，也避免 API key 过期或抄错导致 401。</li>
  </ul>
  <h3>加密保存、解锁、锁定</h3>
  <ul>
    <li><b>加密保存到 VPS</b>：浏览器先用你填写的“本地加密密码”通过 PBKDF2 + AES-GCM 加密整份凭证，再把密文上传到 VPS。VPS 磁盘上保存的是密文，不保存明文私钥。</li>
    <li><b>解锁到 VPS 内存</b>：你输入本地加密密码后，后端把密文解开，并把明文凭证暂存在当前后端进程内存里。之后手动下单、刷新持仓、报告、策略任务才能调用 Polymarket API。</li>
    <li><b>锁定按钮</b>：清除该用户在后端内存里的明文凭证。锁定后，磁盘密文仍在，但下单、持仓刷新、报告生成会要求重新解锁。重启 VPS 后端服务也会自然清空内存，需要重新解锁。</li>
    <li><b>本地加密密码</b>：不是登录密码，也不会存到服务器。忘记后无法解开已保存密文，只能重新填写凭证并覆盖保存。</li>
  </ul>
  <h3>多用户安全隔离</h3>
  <ul>
    <li><b>账号隔离</b>：每个用户有独立登录账号、独立会话 token、独立设置；管理员账号只做后台管理，不能交易。</li>
    <li><b>凭证隔离</b>：每个用户的加密 vault 存在独立路径，解锁后的明文也按用户名分别放在内存中，互相不能读取。</li>
    <li><b>任务隔离</b>：策略任务 id 带用户名，同一个用户只能管理自己的模拟/实盘任务；管理员后台只查看整体状态。</li>
    <li><b>通知隔离</b>：方糖、Telegram、日报时间和模型配置都按用户保存，A 用户不会用到 B 用户的通知渠道。</li>
    <li><b>注册隔离</b>：新用户注册后默认 pending，必须管理员批准后才能登录和保存凭证。</li>
  </ul>
  <h3>通知和模型</h3>
